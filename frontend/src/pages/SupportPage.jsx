...
const SupportPage = () => {
  ...
  return <div className="case-page"><CaseStageHeader item={item} breadcrumb="복구 심사 / 지원금 심사" title="지원금 심사" description="피해등급과 산정 기준을 확인하고 최종 검토 금액을 결정합니다." /><ReviewGuidance current="예상 지원금 계산과 중복 수혜 결과 검토" next="검토 금액 반영 후 최종 승인" caution={item.duplicate ? '중복 수혜 의심 건입니다. 지급 이력 원문을 확인하기 전 최종 승인하지 마세요.' : '산정 기준과 계산 과정이 일치하는지 확인해 주세요.'} /><section className="stage-two-column support-grid"><SupportCalculationCard item={item} standard={SUPPORT_STANDARD} amount={amount} reason={reason} onAmountChange={setAmount} onReasonChange={setReason} onSave={save} error={error} message={message} /><DuplicateBenefitCheck duplicate={item.duplicate} result={item.duplicate ? '동일 주소의 기존 수혜 내역이 있어 추가 확인이 필요합니다.' : SUPPORT_STANDARD.duplicateResult} /></section><StageNavigation previousPath={`/cases/${caseId}/severity`} previousLabel="심각도 검토" nextPath={`/cases/${caseId}/final-approval`} nextLabel="최종 승인" /></div>;
};
