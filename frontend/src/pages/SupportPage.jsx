...
const SupportPage = () => {
  ...
  const hasUnappliedChanges = Number(amount) !== Number(saved.supportAmount) || reason !== saved.supportReason;
  const canProceed = Boolean(saved.supportConfirmed) && !hasUnappliedChanges;

  if (!item) return <div className="case-page"><section className="case-card missing-case"><h1>신고 정보를 찾을 수 없습니다</h1></section></div>;
  const changeAmount = (value) => { setAmount(value); setMessage(''); };
