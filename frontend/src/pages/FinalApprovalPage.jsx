...
const FinalApprovalPage = () => {
  ...
  const gradeCode = String(savedDamageGrade || '').match(/DS[0-4]/i)?.[0]?.toUpperCase();
  const isRejectedGrade = ['DS0', 'DS1', 'DS2'].includes(gradeCode);
  const urgencyScore = isRejectedGrade ? 0 : calculateSeverityTotal(workflow.severityScores);
  ...
  const finalSupportAmount = isRejectedGrade
    ? 0
