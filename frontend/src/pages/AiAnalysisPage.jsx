...
const AiAnalysisPage = () => {
  ...
  }, [analysis.result, analysis.status, caseId, navigate]);
  ...
  return <div className="case-page"><header className="case-page-head"><div><p>AI 분석 / 진행 상태</p><h1>AI 피해 분석</h1><span>신고 이미지의 피해 영역과 등급 분석 진행 상태를 확인합니다.</span></div><button type="button" className="secondary-action" onClick={() => navigate(`/cases/${caseId}`)}>신고 상세로 돌아가기</button></header><CaseProgressStepper /><ReviewGuidance current="AI 분석 요청 및 진행 상태 확인" next="분석 완료 후 신고 상세에서 결과 확인" caution="진행 중에는 화면을 닫아도 작업은 유지되고, 완료 후 통합 신고 상세 화면으로 이동합니다." /><section className="case-card analysis-case-summary"><div><span>사건번호</span><strong>{item.id}</strong></div><div><span>피해 위치</span><strong>{item.location}</strong></div><div><span>재난 유형</span><strong>{formatDisasterType(item.type)}</strong></div></section><AnalysisStatus analysis={analysis} onRequest={() => requestAnalysis(caseId)} onReview={() => navigate(`/cases/${caseId}`)} /></div>;
};
