...
const SeverityPage = () => {
  ...
  return <div className="case-page"><CaseStageHeader item={item} breadcrumb="복구 심사 / 복구 긴급도" title="복구 긴급도" /><ReviewGuidance current="항목별 점수와 복구 긴급도 검토" next="점수 반영 후 지원금 산정 및 중복 수혜 확인" caution="AI 추천 점수를 변경하면 수정 사유가 필수이며 처리 이력에 남습니다." /><section className="stage-two-column"><SeverityScoreTable scores={scores} onScoreChange={updateScore} reason={reason} onReasonChange={(value) => { setReason(value); setMessage(''); }} onSave={save} error={error} message={message} /><UrgencySummary total={total} urgency={urgency} /></section><StageNavigation previousPath={`/cases/${caseId}`} previousLabel="AI 분석" nextPath={`/cases/${caseId}/support`} nextLabel="지원금 심사" nextDisabled={!canProceed} nextHint="점수 반영 버튼을 눌러야 지원금 심사로 이동할 수 있습니다." /></div>;
};
