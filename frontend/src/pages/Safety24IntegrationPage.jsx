  ...
  return <div className="case-page safety24-page">
    ...
    {connected && <section className="safety24-content">
      <aside className="case-card safety24-report-list"><div className="section-heading"><div><h2>수신 신고서</h2><p>연동된 사유재산 피해신고 {SAFETY24_REPORTS.length}건입니다.</p></div></div>
        {SAFETY24_REPORTS.map((item) => { const current = editedReports[item.reportId] || item; return <button type="button" key={item.reportId} className={item.reportId === selectedReportId ? 'selected' : ''} onClick={() => selectReport(item.reportId)} aria-label={`${formatDisasterType(current.disasterType)}, ${current.damagePlace}, ${item.receivedAt}`}><span className="safety24-list-row-head"><strong>{formatDisasterType(current.disasterType)}</strong><time>{item.receivedAt}</time></span><span className="safety24-list-location">{current.damagePlace}</span></button>; })}
      </aside>
