...
const ApprovalHistoryPage = () => {
  ...
  return <div className="case-page approval-history-page">
    ...
    <section className="case-card approval-history-section"><div className="history-section-head"><div><h2>최종 승인 처리</h2><p>삭제한 이력은 현재 화면에서 제외되며, 상단 건수도 즉시 다시 계산됩니다.</p></div></div>
      ...
      <div className="case-table-wrap"><table className="case-table approval-log-table"><thead><tr><th>처리 일시</th><th>사건번호</th><th>신고자 / 위치</th><th>처리 결과</th><th>처리자</th><th>최종 금액</th><th>처리 사유</th><th>관리</th></tr></thead><tbody>{logs.map((log) => { const officer = log.officer || getCurrentUser(); const historyPath = `/cases/${log.caseId}/final-approval?view=history`; return <tr key={log.caseId} onClick={() => navigate(historyPath)} className="clickable-row"><td>{log.processedAt || '기록 없음'}</td><td><strong>{log.caseId}</strong></td><td><strong>{log.case?.reporter || '-'}</strong><small>{log.case?.location || '-'}</small></td><td><span className={`approval-status-badge ${log.status.replaceAll(' ', '-')}`}>{log.status}</span></td><td><strong>{formatOfficerName(officer)}</strong><small>{formatOfficerAffiliation(officer)}</small></td><td>{Number(log.amount || 0).toLocaleString('ko-KR')}원</td><td className="approval-reason">{log.reason}</td><td><div className="history-row-actions"><button className="row-action" onClick={(event) => { event.stopPropagation(); navigate(historyPath); }}>상세</button><button className="history-delete" onClick={(event) => removeHistory(event, log)}>삭제</button></div></td></tr>; })}</tbody></table>{!logs.length && <p className="empty-case">선택한 조건에 해당하는 최종 승인 처리 이력이 없습니다.</p>}</div>
    </section>
  </div>;
};
